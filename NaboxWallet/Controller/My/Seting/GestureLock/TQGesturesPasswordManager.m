...
+ (BOOL)haveLocked{
    if ([[NSUserDefaults standardUserDefaults] stringForKey:TQGesturesPasswordStorageKey]) {
        return YES;
    }else{
        return NO;
    }
...
+ (void)removeLock{
    [[NSUserDefaults standardUserDefaults] setObject:nil forKey:TQGesturesPasswordStorageKey];
}
