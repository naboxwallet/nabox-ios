...
-(void) createSwipeViewIfNeeded
{
    ...
    if (!_leftView && _leftButtons.count > 0) {
        ...
        _leftView.cell = self;
        _leftView.frame = CGRectMake(-_leftView.bounds.size.width + safeInsets.left * ([self isRTLLocale] ? 1 : -1) + self.setLeftOrRightViewOffict,
        ...
        if (self.setCircleRadius) {
            UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:_leftView.bounds byRoundingCorners:UIRectCornerTopRight|UIRectCornerBottomRight cornerRadii:CGSizeMake(self.setCircleRadius, _leftView.frame.size.height)];
    ...
    if (!_rightView && _rightButtons.count > 0) {
        ...
        _rightView.frame = CGRectMake(_swipeOverlay.bounds.size.width + safeInsets.right * ([self isRTLLocale] ? 1 : -1) - self.setLeftOrRightViewOffict,
        ...
        if (self.setCircleRadius) {
            UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:_rightView.bounds byRoundingCorners:UIRectCornerTopLeft|UIRectCornerBottomLeft cornerRadii:CGSizeMake(self.setCircleRadius, _rightView.frame.size.height)];
            CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
